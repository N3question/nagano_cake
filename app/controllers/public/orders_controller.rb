class Public::OrdersController < ApplicationController
  ...
  def new
    @order = Order.new
    @addresses = current_customer.addresses.all #追加
  end
  ...
  def confirm
    ...
    @order.payment_method = params[:order][:payment_method]
		...
		if params[:order][:address_select] == "0"
			...
			@sta = params[:order][:customer_id].to_i
			@order_address = Address.find(@sta)
			@order.postal_code = @order_address.postal_code
			@order.address = @order_address.address
			@order.name = @order_address.last_name + current_customer.first_name #修正
...
　　  @order_detail.order_id =  @order.id #order注文idを紐付けておく
      ...
      @order_detail.amount = cart_item.count #カート内商品の個数を注文商品の個数に代入
